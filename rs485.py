...
def changeEmissivity():
    ...
    res=crc16([0x01, 0x06, 0x9C, 0x41, 0x00, Emissivity]) 
    ser.write(bytearray([0x01, 0x06, 0x9C, 0x41, 0x00, Emissivity, res[0], res[1]]))
    num =1
    while True:
        line = ser.read()
        if(num==6):
        ...
        num= num+1
def getEmissivity():
    res=crc16([0x01, 0x03, 0x9C, 0x41, 0x00, 0x01])
    ser.write(bytearray([0x01, 0x03, 0x9C, 0x41, 0x00, 0x01, res[0], res[1]]))
    num = 1
    while True:
        line = ser.read()
        if(num==5):
            print('16진수 방사율',line)
            print('방사율은', int.from_bytes(line, byteorder='big', signed=True))
        if(num==7):
            break
        num= num+1
